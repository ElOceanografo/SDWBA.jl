...
api_directory = "api"
modules = Docile.Collector.submodules(SDWBA)
...
cd(dirname(@__FILE__)) do
    ...
    open(joinpath(api_directory, "README.md"), "w") do f
        print(f, """
        Files in this directory are generated using the `generate_docs.jl` script. Make
    ...
    info("Need to build website using mkdocs from SDWBA root:")
